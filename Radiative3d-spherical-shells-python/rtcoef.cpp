...
//             RTCoef::GetCoefs_SV()
//
//   Computes the coefficients needed for interface conversion into ground motion.
//    The coefficients are computed using the formulae in Cerveny's 1985 paper.
//    The coefficients are computed for the case of a seismometer located at any interface.
//    
void RTCoef::SetCervenyConversionCoef(raytype intype) {
  ...
  Real p = mSini / VelocR[rtyp];

  Real p_sq = p*p;
  
  // Compute Outgoing Sines from Incident Sine:

  Real q = 2* (rho2*(beta2*beta2) - rho1*(beta1*beta1));
  Real q_sq = q*q;
  Real X = rho2 - q*(p_sq);
  Real Y = rho1 + q*(p_sq);
  Real Z = rho2 - rho1 - q*(p_sq);

  // Complex P functions
  Complex P1 = ComputePRoot(((1- (alpha1*alpha1)*(p_sq))));  
  Complex P2 = ComputePRoot((1- (beta1*beta1)*(p_sq)));
  Complex P3 = ComputePRoot((1- (alpha2*alpha2)*(p_sq)));
  Complex P4 = ComputePRoot((1- (beta2*beta2)*(p_sq)));

  // Compute the remaining parameters:

  Complex D = (q_sq*p_sq)*P1*P2*P3*P4 
                + (rho1*rho2)*(beta1*alpha2*P1*P4 + beta2*alpha1*P2*P3)
                + alpha1*beta1*P3*P4*(Y*Y) 
                + alpha2*beta2*P1*P2*(X*X)
                + alpha1*alpha2*beta1*beta2*(p_sq)*Z;

  Complex Dbar= (rho1*beta1*P2) + (rho2*beta2*P4);


  // Set the conversion coefficients, without the orientation index !! 
  //Here, we use the formula for the transmission coefficients in Cerveny's notation Dij(R+).
  // If the seismometer is at the free surface we simply set the elastic parameters of the transmitted side to zero.

  mCerveny.D11 = (2* rho1*beta1*P2/D)*(rho2*alpha2*P1*P4+alpha1*P3*P4*Y - alpha1*alpha2*beta2*p_sq*Z); // Conversion coefficient for SV to i1
  mCerveny.D13 = (2*rho1*alpha1*p*P1/D)*(rho2*beta1*alpha2*P4 + alpha2*beta2*P2*X - q*P2*P3*P4); // Conversion coefficient for SV to i3
  mCerveny.D22 = (2*rho1*beta1*P2)/Dbar; // Conversion coefficient for SH to i2
  mCerveny.D31 = (2*rho1*beta1*p*P2/D)*(q*P1*P3*P4 - alpha2*beta2*P1*X - rho2*beta2*alpha1*P3); // Conversion coefficient for P to i1
  mCerveny.D33 = (2*rho1*alpha1*P1/D)*(rho2*beta2*P2*P3+ beta1*P3*P4*Y - alpha2*beta1*beta2*p_sq*Z); // Conversion coefficient for P to i3
}
...
//    The coefficients are computed for the case of a seismometer located at any interface.
//    
void RTCoef::SetCervenyConversionCoefFreeSurface(raytype intype) {
 ...
 //                          : RAY_S;   // just plain S
  ...
  // Horizontal Slowness:
  Real p = mSini / VelocR[intype]; // Should work since VelocR is delcarated as an array of size 3
  ...
  // Compute complex P functions

  switch (intype) {
  case RAY_P: {

  Complex P1 = ComputePRoot((alpha1 * alpha1) * p_sq);
  Complex P2 = ComputePRoot( (beta1 * beta1) * p_sq);
  Complex D1 = std::pow(1 - 2 * (beta1 * beta1) * p_sq, 2) 
            + (4 * (beta1 * beta1 * beta1) * p_sq * P1 * P2 / alpha1); // Rayleigh function

    mCerveny.D11 = 0;
    mCerveny.D13 = (4 * beta1 * p * P1 * P2 / D1);
    mCerveny.D31 = 0;
    mCerveny.D33 = (2*(1 - 2 * (beta1 * beta1) * p_sq)*P2) / D1;
    mCerveny.D22 = 0.0;
    break;
  }
  case RAY_SV: {
    ...
    mCerveny.D11 =   (2 *(1 - 2 * (beta1 * beta1) * p_sq)*P2) / D1;
    mCerveny.D31 = (-4 * (beta1 * beta1) * p * P1 * P2 / (alpha1 * D1));
    mCerveny.D13 = 0;
    mCerveny.D33 = 0;
    mCerveny.D22 = 0.0;
    break;
