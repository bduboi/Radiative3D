...
void process_option(CmdOpt & opt, ModelParams & params, 
  ...
  //std::cout << "DEBUG: Option ID: " << opt.GetID() << std::endl;
  ...
  ////////////////////////////////////////////////////////////////////
  case CmdOpt::OPT_OUTPUT_ENVELOPE: // *** Output Envelope Data
                                    // ***
    std::cout << "Simulation will output envelope data.\n";
    params.mOutputEnergy = opt.PopValue_Real();
    break;


  ////////////////////////////////////////////////////////////////////
  case CmdOpt::OPT_OUTPUT_DISP: // *** Output Displacement Data
                                // ***
    std::cout << "Simulation will output displacement data.\n";
    params.mOutputDisplacement  = opt.PopValue_Real();

    if (params.mOutputDisplacement & params.mOutputEnergy) {
      std::cout<< "Both displacement and energy output requested.\n";
    }
    break;
  ...
  case CmdOpt::OPT_COMPSELECT:  // *** Grid-Compiled Model Selector:
    ...
    if (params.CompiledSelector == 666){
      params.GridSource = ModelParams::GRID_FROMFILE;
      break;
    }
  ...
  ///////////////////////////////////////////////////////////////////
  case CmdOpt::OPT_MODPATH:        // *** Path for the model to be read.
  //
  std::cout << "DEBUG: Processing option " << opt.GetOptionText() 
          << " with ID = " << opt.GetID() << std::endl;
      params.ModelPath = opt.PopValue_Text();
  
      if (params.ModelPath.empty()) { // Set default path to prem if user chooses "".
        params.ModelPath = "/Models/EarthModels/PREM/Simplified.csv";}
  break;
