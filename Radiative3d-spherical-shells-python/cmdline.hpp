...
public:
  ...
  enum cmdopts_e {      // Provides ID codes for each option type
    ...
    OPT_COMPSELECT,     // Model-compiled selector index : value defines whether we use a compiled model OR a Model-file (666 is model file)
    ...
    OPT_MODPATH,        // Path for the model to be read.
    ...
    OPT_OUTPUT_ENVELOPE,// Output envelope data
    OPT_OUTPUT_DISP,    // Output displacement data
...
CmdOpt::OpMap CmdOpt::init_map() {
  ...
  map["--model-path"]  =        OPT_MODPATH;  // Path of the Model used 
  ...
  map["--output-envelope"] =OPT_OUTPUT_ENVELOPE;  // Output envelope data
  map["--output-displacement"] =    OPT_OUTPUT_DISP;  // Output displacement data
