...
C3::ComplexMatrix Phonon::GetConversionMatrix(const CellFace * pFace,const Real & epsilon,bool isFreeSurface) const{
  //std::cout << "Entered GetConversionMatrix" << std::endl;
  RTCoef rt = pFace->GetRTBasis(mLoc, mDir);  
                      // Gets basis vectors for scattered
                      // rays and velocities, densities
  ...
  // Check if collection face is at free surface
  if (isFreeSurface){
    //std::cout << "Set the Free Surface conversion params" << std::endl; CHECKED THAT IT WORKS !
    rt.SetCervenyConversionCoefFreeSurface(intype);  // Set Cerveny conversion parameters in the free surface case (alpha2,beta2,rho2 = 0)
  }
  else{
    rt.SetCervenyConversionCoef(intype);  // Set Cerveny conversion parameters
  }
  C3::ComplexMatrix D = C3::ComplexMatrix(epsilon* rt.mCerveny.D11,0,rt.mCerveny.D13,
                                      0,rt.mCerveny.D22,0,
                                      rt.mCerveny.D31,0,epsilon* rt.mCerveny.D33);                   
...
void Phonon::Propagate() {
  ...
  while (true) {    // ===========================
    ...
    if (travel.pFace->IsCollectionFace()) { 
      ...
      dataout.ReportPhononCollected(*this, travel.pFace);
