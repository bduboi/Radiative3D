...
protected:
  ...
  public:            // seismic trace. Energy is binned by time window
    ...
    //Real mDisplacementAxisReal[NUM_AXES];       // Displacement by axis
    //Real mDisplacementAxisImag[NUM_AXES];       // Displacement by axis
    ...
    BinRecord() {               // Make sure bins get initialized
      ...
      //mDisplacementAxisReal[AXIS_X] = 0;
      //mDisplacementAxisReal[AXIS_Y] = 0;
      //mDisplacementAxisReal[AXIS_Z] = 0;
      //mDisplacementAxisImag[AXIS_X] = 0;
      //mDisplacementAxisImag[AXIS_Y] = 0;
      //mDisplacementAxisImag[AXIS_Z] = 0;
...
protected:
  ...
  Real    mVolume[RAY_NBT];  // Volume of the gather region. Used for
                              // dimensional scaling of displacement bins.                                
  ...
  static bool         mOutputDisplacement;  // True if we want to output displacement. This will change the way CatchPhonon funciton work

  static bool         mOutputEnergy;  // True if we want to output energy

  static bool        isFreeSurface; // True if the seismometer is located at free surface
...
public:
  ...
  Seismometer(const R3::XYZ loc,        // Seismometer Location
              ...
              std::string axes_desc = "UNK",const bool mOutputEnergy=false, const bool mOutputDisplacement=true);
  ...
  static void SetFreeSurface(bool output) {
    isFreeSurface = output;
  }
  // :::::::::::::::::::::::::::::::::::::::::::::::::
  // ::: Do-Something Methods  (Seismometer Class) :::
  // :::::::::::::::::::::::::::::::::::::::::::::::::

  /* Old CatchPhonon Function is now deprecated. The new function has an option to work like the old one.
  ...
  */
  ...
  bool CatchPhonon(const Phonon & phon, const CellFace * pFace);
  //            Checks whether the Phonon is within the gather radii,
  //            and if it is, we ingest it and increment the appropriate
  //            bins.  Returns true if we ingested the phonon, and false
  //            if we either (a) did NOT ingest or (b) we DID ingest,
...
public:
  ...
  void AddSeismometer(R3::XYZ loc, R3::XYZ axisX1,
      ...
      mSeismometers
        .push_back(new Seismometer(loc, axisX1, 
                                   inner_radius, 
                                   outer_radius, axes_desc, mOutputEnergy, mOutputDisplacement));
  }
  ...
  void ReportPhononCollected(const Phonon &, const CellFace *);
                                      // Phonon arrival at collection face : Takes the pFace for converting to displacement
    
  //void ReportPhononCollectedDisp(const Phonon &,const CellFace *);
