...
public:
  ...
  R3::XYZ VecSquared() const {
    return R3::XYZ(mX*mX, mY*mY, mZ*mZ);
  }
  R3::XYZ VecSqrt() const {
    return R3::XYZ(std::sqrt(mX), std::sqrt(mY), std::sqrt(mZ));
  }
  ...
  const R3::XYZ VecPhase(const R3::XYZ &other) const {
    return R3::XYZ(std::atan2(other.x(), x()),
                   std::atan2(other.y(), y()),
                   std::atan2(other.z(), z()));
}
...
public:
  ...
  Matrix(const R3::XYZ& row1, const R3::XYZ& row2, const R3::XYZ& row3)
  : mxx(row1.x()), mxy(row1.y()), mxz(row1.z()),
    myx(row2.x()), myy(row2.y()), myz(row2.z()),
    mzx(row3.x()), mzy(row3.y()), mzz(row3.z()) {}
