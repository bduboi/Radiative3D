...
public:
  ...
  struct CervenyParams{
    Complex D11; // Conversion coefficient for SV to i1
    Complex D13; // Conversion coefficient for SV to i3
    Complex D22; // Conversion coefficient for SH to i2
    Complex D31; // Conversion coefficient for P to i1
    Complex D33; // Conversion coefficient for P to i3

  };   

  CervenyParams mCerveny; // Cerveny parameters for the surface conversion coefficients                                 
...
public:
  ...
  // :::::::::::::::::::::::::::::::::::::::::::
  void SetCervenyConversionCoef(raytype); // Helper function to compute the intermediate parameters needed for the Cerveny conversion coefficients
  void SetCervenyConversionCoefFreeSurface(raytype); // Modified function for free surface receiver (alpha2, beta2, rho2 = 0)
...
private:
  ...
  void GetOrientationIndex(); // Helper function to compute the orientation index based on forward or backward propagation which is defined as sign( phondir \cdot fnorm)



  Complex ComputePRoot(double value) {
    if (value > 1.0) {
        return Complex(0.0, std::sqrt(value - 1.0)); // Purely imaginary root
    } else {
        return Complex(std::sqrt(1.0 - value), 0.0); // Purely real root
    }
  }
