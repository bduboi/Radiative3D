...
Model::Model(const ModelParams & par) {
  ...
  switch ( par.GridSource ) {
    ...
    case ModelParams::GRID_FROMFILE:    // Grid comes from file
      ...
      std::cout << "|  Grid source: File Model from path : "
                << par.ModelPath << std::endl;
      try {mGrid.GridFromFile(par.ModelPath);}
  ...
  for ( std::vector<ModelParams::SeisRequest>::const_iterator 
    ...
    //
      ...
      dataout.AddSeismometer(Location, SeisAxisX1, rad_in, rad_out, AxesDesc,par.mOutputEnergy,par.mOutputDisplacement);
    //
